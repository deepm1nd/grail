...
- [ ] `espup` installed at the version pinned in `scripts/setup_env.sh` (`0.14.1`, see
      Version Reference table) — **not** whatever `cargo install espup --locked` resolves
      to unpinned — and `espup install` completed
...
```bash
# Install the esp toolchain (Xtensa + RISC-V) — pinned, never unpinned `latest`:
cargo install espup --version 0.14.1 --locked
espup install
...
**Do not install `espup` unpinned.** An unpinned `espup install` resolves whatever release
...
producing a full Task Group TG-0001 escalation. Always pin the exact `espup` version in
