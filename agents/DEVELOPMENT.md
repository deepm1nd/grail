...
4.  **Pre-Commit Verification & Quality Assurance:**
    -   **Documentation:** Verify that all documentation is up-to-date per the **Mandate for Pre-Commit Documentation Integrity**.
...
5.  **Commit Phase Changes:** After all tests have passed and the **Phase-End Quality Assurance** is complete, the agent MUST commit (finalize and submit) all changes. This is a mandatory checkpoint to prevent work loss from session crashes.
